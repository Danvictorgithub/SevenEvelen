...
export class XenditService {
    ...
    async checkout(checkOutDto: CheckOutDto, userId: number) {
        ...
        const products = await this.prisma.cartItem.findMany({ where: { id: userId, productId: { in: checkOutDto.products } }, include: { product: { include: { product: true } }, user: true } });
        const { data } = await axios.post<XenditInvoiceResponse>("https://api.xendit.co/v2/invoices", {
            external_id: `${products.map(p => p.id).join("-")}`,
            description: "UserCheckout",
            amount: products.reduce((acc, cur) => acc + (cur.product.product.originalPrice + (cur.product.markupRate / 100 * cur.product.product.originalPrice)), 0),
            currency: "PHP",
            customer: { "email": user.email },
            customer_notification_preference: {
                "invoice_paid": ["email"],
